...
WILLINGNESS_INSTRUCTION = "## INSTRUKCJA - Oceń swoją osobistą gotowość do podjęcia następującego działania, używając skali - 1 całkowicie niechętnie, 2 raczej niechętnie, 3 nieco niechętnie, 4 neutralnie, 5 nieco chętnie, 6 raczej chętnie, 7 całkowicie chętnie:"
...
ARTICLES = [
    Article(
        ...
        "Zgodnie z nowym raportem FutureNow siła nabywcza spadnie w nadchodzących latach. Rzecznik niezależnej fundacji, która od lat monitoruje rozwój gospodarczy, komentuje raport: „Musimy podnieść świadomość na temat tego, co oznacza ten prospekt. Będzie mniej pieniędzy do wydania. Należy teraz podjąć działania, aby zająć się tym zagrożeniem."
    ),
    Article(
        "Siła nabywcza spadnie dla [] obywateli - Fundacja FutureNow obwinia polityków w nowym raporcie",
        "<opis - ręce i otwarty pusty portfel, właściciel patrzy w środku>",
        "Zgodnie z nowym raportem FutureNow siła nabywcza w [] spadnie w nadchodzących latach. Rzecznik niezależnej fundacji, która od lat monitoruje rozwój gospodarczy, komentuje raport: „Przeciętni obywatele w [] muszą zostać poinformowani o tym, że będą mieli mniej pieniędzy do wydania. Tak wielu ludzi w [] tak ciężko pracuje, aby mieć dobre życie. Jest coś głęboko złego, gdy te wysiłki są bezowocne. Oczywiste jest, że winni są politycy. W ostatnich latach byli zbyt krótkowzroczni, samolubni i skorumpowani. Nie dbają o nikogo oprócz siebie i są zbyt oderwani od rzeczywistości. Należy teraz podjąć działania, aby zająć się tym zagrożeniem dla dobrobytu naszego ludu."
    ),
    Article(
        "Siła nabywcza spadnie dla [] obywateli - Fundacja FutureNow obwinia imigrantów w nowym raporcie",
        "<opis - ręce i otwarty pusty portfel, właściciel patrzy w środku>",
        "Zgodnie z nowym raportem FutureNow siła nabywcza w [] spadnie w nadchodzących latach. Rzecznik niezależnej fundacji, która od lat monitoruje rozwój gospodarczy, komentuje raport: „Przeciętni obywatele w [] muszą zostać poinformowani o tym, że będą mieli mniej pieniędzy do wydania. Tak wielu ludzi w [] tak ciężko pracuje, aby mieć dobre życie. Jest coś głęboko złego, gdy te wysiłki się nie opłacają. Oczywiste jest, że winni są imigranci. Są zbyt wymagający, wykorzystują nasz system i trudno ich zintegrować. Należy teraz podjąć działania, aby zająć się tym zagrożeniem dla dobrobytu naszego ludu."
    ),
    Article(
        "Siła nabywcza spadnie dla [] obywateli - Fundacja FutureNow obwinia polityków i imigrantów w nowym raporcie",
        "<opis - ręce i otwarty pusty portfel, właściciel patrzy w środku>",
        "Zgodnie z nowym raportem FutureNow siła nabywcza w [] spadnie w nadchodzących latach. Rzecznik niezależnej fundacji, która od lat monitoruje rozwój gospodarczy, komentuje raport: „Przeciętni obywatele w [] muszą zostać poinformowani o tym, że będą mieli mniej pieniędzy do wydania. Tak wielu ludzi w [] tak ciężko pracuje, aby mieć dobre życie. Jest coś głęboko złego, gdy te wysiłki są bezowocne. Oczywiste jest, że winni są politycy i imigranci. W ostatnich latach politycy byli zbyt krótkowzroczni, samolubni i skorumpowani. Imigranci są zbyt wymagający, wykorzystują nasz system i trudno ich zintegrować. I jednak politycy zajmują się tylko imigrantami zamiast naszym ludem. Należy teraz podjąć działania, aby zająć się tym zagrożeniem dla dobrobytu naszego ludu."
    )
]
